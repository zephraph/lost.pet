...
import manifest, { CloudAssetManifest } from "../cloud-assets/manifest";
 ...
 */
async function processAsset(filePath: string, manifest: CloudAssetManifest, hashToName: Record<string, string>, force: boolean = false) {
  ...
  if (manifest[file]) {
    ...
    for (const variant of manifest[file].variants) {
      ...
      const variantNeedsUpdate = !existingVariant ||
        !existingVariant.parent ||
        existingVariant.parent !== file ||
        !existingVariant.transform ||
        JSON.stringify(existingVariant.transform) !== JSON.stringify(transform) ||
        force;
      ...
      if (variantNeedsUpdate) {
        ...
        const variantContentType = mime.getType(variantPath) || "application/octet-stream";
...
if (import.meta.url.startsWith("file:")) {
  ...
  if (process.argv[1] === modulePath) {
    ...
    if (updated) {
      ...
      if (begin === -1 || end === -1) throw new Error("Manifest delimiters not found");
